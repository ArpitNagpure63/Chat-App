...
const getConversation = async (req, res) => {
    try {
        ...
        const conversations = await Conversations.find({ usergroup: { $in: [senderId] } });
        ...
        const userConversations = await Users.find({ _id: { $in: userConversationsIDs } }).select('-password').select('-username');
